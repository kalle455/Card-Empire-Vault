...
export default function AccountPanel() {
  ...
  async function loadPlayerRecords() {
    ...
    const [orderResult, wishResult] = await Promise.all([
      ...
      supabase.from("wishlists").select("id,created_at,availability_notifications,card:cards(id,name,image_url,ygo_card_id,rarity,category,price,quantity)").eq("player_id", session.user.id).order("created_at", { ascending: false }),
    ]);
  ...
  return <section className="profile-command">
    ...
    {tab === "wishlist" && <section className="profile-record-list wishlist-record-list"><header><div><p className="overline">WISHLIST WATCH</p><h2>Cards on your radar.</h2><p className="wishlist-discord-note"><i /> Discord alerts active for availability changes</p></div><span>{wishlist.length} watched</span></header>{wishlist.map((wish) => <article key={wish.id}><span className="profile-record-art">{imageFor(wish.card) && <img src={imageFor(wish.card)} alt="" />}</span><div><small>{wish.card?.rarity} · {wish.card?.category}</small><h3>{wish.card?.name ?? "Card"}</h3><p className={wish.card?.quantity > 0 ? "is-available" : "is-sold"}>{wish.card?.quantity > 0 ? `${wish.card.quantity} available now` : "Currently sold"}</p></div><strong>{Number(wish.card?.price ?? 0).toLocaleString()} G</strong><button type="button" onClick={() => removeWish(wish.id)}>Remove</button></article>)}{!wishlist.length && <p className="profile-empty">Save a card in Cardstock and it will appear here. Discord alerts will notify you when its availability changes.</p>}</section>}
